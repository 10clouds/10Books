...
class Library extends PureComponent {
        ...
        const rating =
          product.ratings.length > 0
            ? (
                ...
                }, 0) / parseFloat(product.ratings.length).toFixed(1)
              ).toString()
            : null
  ...
  getTableRowProps = product => ({
    isHighlighted:
      product.used_by && product.used_by.user.id === this.props.currentUser.id
  })
    ...
    return (
      <Fragment>
        ...
        <RateProductModal
          ...
          onSubmit={value => {
            this.props.libraryActions.rateProduct(
              this.state.rateProductWithId,
              value
            )
...
const mapDispatchToProps = dispatch => {
  return {
    ...bindActionCreators(
      { joinProductsChannel, joinCategoriesChannel },
      dispatch
    ),
