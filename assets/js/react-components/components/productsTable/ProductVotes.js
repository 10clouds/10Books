...
export default class ProductVotes extends PureComponent {
  ...
  render() {
    const { product, currUser, upvote, downvote } = this.props
    ...
    const canVote =
      product.requested_by.id !== currUser.id && product.status === 'REQUESTED'
      ...
      >
        ...
        >
          <Raw
            ...
            content={require('/static/images/product-votes-counter__arrow.svg')}
          />
        </button>
        ...
        <button
          ...
          className={cn('product-votes__vote-btn', {
            ...
            'product-votes__vote-btn--active':
              canVote && currUserDidUpvote === false
          })}
